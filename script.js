...
loader.load('https://threejs.org/examples/fonts/helvetiker_regular.typeface.json', function (font) {
  const textGeometry = new THREE.TextGeometry('Happy Coding!', {
    font: font,
    size: 30,
    height: 5,
    curveSegments: 12,
  ...
  const textMaterial = new THREE.MeshBasicMaterial({ color: 0xee5282 });
  const textMesh = new THREE.Mesh(textGeometry, textMaterial);
  textMesh.position.set(-100, 0, 0); // Adjust as needed
  scene.add(textMesh);
});
